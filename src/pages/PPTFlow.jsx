...
const SCENES = [
  {
    id: 'popular_science', icon: '🎬', title: '科普演讲',
    desc: '全屏视觉冲击，TED/Keynote 风格',
    detail: '全屏背景图+大字标题 · 每页 1-2 要点 · 暗色/渐变',
    available: true,
  },
  {
    id: 'lab_report', icon: '🔬', title: '课题汇报',
    desc: '信息密集，图表为主，支持校模板',
    detail: '数据+图表+要点 · 论文原图 · 亮色/白底',
    available: true,
  },
  {
    id: 'conference', icon: '📊', title: '学术会议',
    desc: '严肃规范，大量引用，学术风',
    detail: '方法+结果+引用 · 论文原图+流程图 · 白底/浅色',
    available: false,
  },
  {
    id: 'thesis_defense', icon: '🎓', title: '毕业答辩',
    desc: '清晰条理，逻辑清晰，平衡美观',
    detail: 'AI 概念图+论文原图混合 · 学校色系',
    available: false,
  },
  {
    id: 'business_pitch', icon: '💼', title: '企业路演',
    desc: '数据亮点突出，高端质感，商业叙事',
    detail: 'AI 概念图+数据可视化 · 暗色/渐变',
    available: false,
  },
];
...
function SceneSelect({ onSelect }) {
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '20px 24px' }}>
      ...
      <div style={{
        display: 'flex', gap: 14, overflowX: 'auto', padding: '0 0 12px',
          ...
          >
            <div style={{
              ...
              background: s.available
                ? s.id === 'popular_science'
                  ? 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)'
                  : 'linear-gradient(135deg, var(--card2) 0%, var(--bg2) 100%)'
                : 'var(--bg2)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 36,
            }}>{s.icon}</div>
            <div style={{ padding: '12px 14px', flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                ...
                {!s.available && (
                  <span style={{
                    fontSize: 8, color: 'var(--text-l)', background: 'var(--bg2)',
  ...
  return (
    <div style={{ maxWidth: 560, margin: '0 auto', padding: '20px 24px' }}>
      ...
      <div style={{
        ...
        background: 'var(--card)', boxShadow: 'var(--shadow)', padding: '20px',
      }}>
        {/* Pages */}
            ...
            {[6, 8, 10, 12].map((n) => (
              <button
                ...
                style={{
                  ...
                  border: pages === n ? '1.5px solid var(--dust)' : '1px solid var(--border)',
                  background: pages === n ? 'var(--dust-bg)' : 'var(--bg)',
                  color: pages === n ? 'var(--dust)' : 'var(--text-m)',
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[
              ...
              <button
                ...
                style={{
                  flex: 1, padding: '10px 4px', borderRadius: 8,
                  border: language === l.id ? '1.5px solid var(--dust)' : '1px solid var(--border)',
                  background: language === l.id ? 'var(--dust-bg)' : 'var(--bg)',
                  color: language === l.id ? 'var(--dust)' : 'var(--text-m)',
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {['16:9', '4:3'].map((r) => (
              <button
                ...
                style={{
                  flex: 1, padding: '10px 4px', borderRadius: 8,
                  border: ratio === r ? '1.5px solid var(--dust)' : '1px solid var(--border)',
                  background: ratio === r ? 'var(--dust-bg)' : 'var(--bg)',
                  color: ratio === r ? 'var(--dust)' : 'var(--text-m)',
                  fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
                }}
              >{r}</button>
            ))}
        ...
        {/* Custom template upload (lab_report only) */}
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{
            ...
            border: `1.5px dashed ${isLabReport ? 'var(--border)' : 'var(--bg2)'}`,
            background: isLabReport ? 'var(--bg)' : 'var(--bg2)',
            textAlign: 'center',
            opacity: isLabReport ? 1 : 0.45,
            cursor: isLabReport ? 'pointer' : 'default',
          }}>
            <div style={{ fontSize: 24, marginBottom: 6 }}>📁</div>
            <div style={{ fontSize: 11, color: 'var(--text-l)' }}>
              {isLabReport
                ? '拖拽 .pptx 文件到此处，或点击上传'
                : '仅「课题汇报」场景可用'}
            </div>
          ...
          {isLabReport && (
            <div style={{ fontSize: 10, color: 'var(--text-l)', marginTop: 6, lineHeight: 1.5 }}>
              上传学校/机构的 PPT 模板，AI 会按照你的模板风格生成内容。
            </div>
          )}
        </div>
        ...
        {/* Submit */}
        <button
          onClick={() => onSubmit({ pages, language, ratio })}
          style={{
...
function PPTGenerating({ config, onDone }) {
  ...
  useEffect(() => {
    ...
    const advanceStage = (idx) => {
      ...
      // For image generation stage, animate page counter
  ...
  return (
    <div style={{ maxWidth: 520, margin: '0 auto', padding: '40px 24px', textAlign: 'center' }}>
      ...
      <p style={{ fontSize: 12, color: 'var(--text-l)', marginBottom: 24 }}>
        {config.pages} 页 · {config.language === 'zh' ? '中文' : config.language === 'en' ? '英文' : '中英双语'} · {config.ratio}
      </p>

      {/* Progress bar */}
      <div style={{ height: 6, background: 'var(--bg2)', borderRadius: 3, marginBottom: 24 }}>
        <div style={{
          height: '100%', borderRadius: 3, background: 'var(--dust)',
          width: `${overallPercent}%`, transition: 'width 0.5s ease',
        }} />
      </div>
        ...
        {GEN_STAGES.map((stage, i) => {
          ...
          return (
            <div key={stage.key} style={{
              ...
              padding: '10px 0',
              borderBottom: i < GEN_STAGES.length - 1 ? '1px solid var(--border)' : 'none',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{
                  ...
                  color: isPending ? 'var(--text-l)' : 'var(--text-d)',
                }}>
                  {stage.label}
                  {isActive && stage.showPages && currentPage > 0 && (
                    <span style={{ color: 'var(--dust)', marginLeft: 6 }}>
                      第 {currentPage}/{config.pages} 页
                    </span>
                  )}
                </span>
              </div>
              <span style={{
                ...
                color: isDone ? 'var(--sage)' : isActive ? 'var(--dust)' : 'var(--text-l)',
              }}>
      ...
      <div style={{ fontSize: 11, color: 'var(--text-l)', marginTop: 16 }}>
        预计剩余时间：~{Math.max(1, Math.round((totalDuration - elapsed) / 1000))} 秒
      </div>
    </div>
  );
}
    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '12px 24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--text-l)' }}>{slides.length} 页 · 点击缩略图选中，点击下方大图全屏查看</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {marked > 0 && (
            ...
            }}>↻ 重新生成 {marked} 页</button>
          )}
          <a href="/demo-samples/llm-agents-deck.pptx" download style={{
            fontSize: 11, color: 'var(--dust)', textDecoration: 'none',
            border: '1px solid var(--dust)', borderRadius: 8, padding: '7px 16px', fontWeight: 700,
      ...
      {/* Thumbnail carousel */}
      <div style={{
        display: 'flex', gap: 10, overflowX: 'auto', padding: '0 0 10px', marginBottom: 12,
          ...
          return (
            ...
            >
              ...
              <div style={{ aspectRatio: '16/9', background: 'var(--bg)', overflow: 'hidden', position: 'relative' }}>
                <img src={s.thumb} alt={`Slide ${s.page}`} loading="lazy"
                  style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                <div style={{
                  position: 'absolute', left: 6, top: 4,
                  fontSize: 10, fontWeight: 700, color: '#fff',
                  textShadow: '0 1px 4px rgba(0,0,0,0.6)',
                }}>{String(s.page).padStart(2, '0')}</div>
              </div>
            </div>
          );
        })}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 280px', gap: 14 }}>
        ...
        <div style={{
          ...
          <img
            src={slides[selectedSlide]?.thumb}
          ...
          <div style={{
            ...
            color: 'var(--text-m)', textAlign: 'center',
          }}>
            第 {selectedSlide + 1} 页 — {slides[selectedSlide]?.title}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{
            ...
            <button
              onClick={() => toggleMark(selectedSlide)}
              style={{
                width: '100%', padding: '9px', borderRadius: 8,
                border: markedSlides.has(selectedSlide) ? '1.5px solid #C45C5C' : '1px solid var(--border)',
                background: markedSlides.has(selectedSlide) ? 'rgba(196,92,92,0.06)' : 'var(--bg)',
                color: markedSlides.has(selectedSlide) ? '#C45C5C' : 'var(--text-m)',
                fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
              }}
            >
              {markedSlides.has(selectedSlide) ? '↻ 已标记重新生成（点击取消）' : '标记此页需要重新生成'}
            </button>
          </div>
          ...
          <div style={{
            borderRadius: 12, padding: '12px 14px',
            border: '1px solid var(--border)', background: 'var(--card)',
            boxShadow: 'var(--shadow)', flex: 1, minHeight: 0, overflow: 'auto',
          }}>
            ...
            {slides.map((s, i) => (
              <div
                ...
                style={{
                  ...
                  background: i === selectedSlide ? 'var(--dust-bg)' : 'transparent',
              ...
              >
                <span style={{
                  fontSize: 9, fontWeight: 700, color: 'var(--text-l)',
                  width: 16, textAlign: 'right', flexShrink: 0,
                }}>{s.page}</span>
                <span style={{
                  fontSize: 10, color: i === selectedSlide ? 'var(--dust)' : 'var(--text-m)',
                  fontWeight: i === selectedSlide ? 700 : 400,
          ...
          <div style={{
            ...
            <div style={{ fontSize: 10, color: 'var(--text-l)', lineHeight: 1.6, marginBottom: 10 }}>
              📌 下载后可在 PowerPoint / WPS / Keynote 中自由编辑文字、替换图片、调整排版。
            </div>
            <a href="/demo-samples/llm-agents-deck.pptx" download style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center',
 ...
 *  主容器：PPTFlow
 * ══════════════════════════════════════════════════ */
export default function PPTFlow({
  onNext,
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      {subStep === 0 && (
        <SceneSelect onSelect={handleSceneSelect} />
      )}
