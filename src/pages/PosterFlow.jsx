        ...
        return (
          <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
            ...
            {i < SUB_STEPS.length - 1 && (
              <div style={{
                width: 40, height: 1.5, marginTop: -12,
...
function StyleSelect({ onSelect }) {
  const styles = [
    {
      id: 'journal', icon: '📖', title: '期刊封面风格',
      desc: '还原 Nature / Cell / Science 等期刊封面排版',
      detail: '竖版为主 · 遵循期刊模板 · 博物馆质感',
      available: true,
    },
    {
      id: 'academic', icon: '📋', title: '学术海报风格',
      desc: '适合学术会议 poster session',
      detail: 'A0 竖版 · 标题+摘要+配图+数据',
      available: false,
    },
    {
      id: 'social', icon: '📱', title: '社交分享图',
      desc: '适合朋友圈 / 小红书 / Twitter',
      detail: '方形或竖版 · 简洁大图+标题',
      available: false,
    },
  ];
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        {styles.map((s) => (
          ...
          >
            {/* Demo preview area */}
            <div style={{
              height: 160, background: s.available
                ? 'linear-gradient(135deg, var(--sage-bg) 0%, var(--dust-bg) 100%)'
                : 'var(--bg2)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 48,
            }}>
            ...
            <div style={{ padding: '16px 18px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                ...
                {!s.available && (
                  <span style={{
                    fontSize: 9, color: 'var(--text-l)', background: 'var(--bg2)',
    ...
    <div style={{ maxWidth: 840, margin: '0 auto', padding: '20px 24px' }}>
      ...
      {JOURNAL_GROUPS.map((group) => (
        <div key={group.group} style={{ marginBottom: 20 }}>
          <div style={{
            fontSize: 11, fontWeight: 700, color: 'var(--text-l)',
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
            {group.items.map((j) => (
              <div
                ...
                onMouseEnter={() => setHoveredJournal(j.id)}
              ...
              >
                ...
                {hoveredJournal === j.id && (
                  <div style={{
                    ...
                    background: '#1E1C1A', color: '#E8E4DC',
      ...
      <div style={{ marginTop: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          />
          <button
            onClick={() => customInput.trim() && onSelect(customInput.trim())}
            disabled={!customInput.trim()}
            style={{
              padding: '10px 20px', borderRadius: 10,
              background: customInput.trim() ? 'var(--sage)' : 'var(--bg2)',
              border: 'none', color: customInput.trim() ? '#fff' : 'var(--text-l)',
              fontSize: 12, fontWeight: 700, cursor: customInput.trim() ? 'pointer' : 'default',
              fontFamily: 'inherit',
            }}
          >确认</button>
        </div>
...
function ProposalSelect({ journal, onSelect, onBack }) {
  ...
  // Simulate AI brainstorming ~2.5s
  useEffect(() => {
    const t = setTimeout(() => {
      setLoading(false);
      setExpandedId(proposals[0].id); // auto-expand top-scored
    }, 2500);
  ...
  if (loading) {
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '40px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--text-l)', marginBottom: 24 }}>
          正在为 <b>{journalLabel}</b> 风格生成 5 个创意方案
        </p>
        {/* Skeleton cards */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} style={{
              ...
              animation: `shimmer 1.8s ease infinite ${i * 0.15}s`,
            }} />
          ))}
        </div>
      </div>
            ...
            >
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  {isTop && <span style={{
                    ...
                    background: 'var(--sage-bg)', padding: '2px 8px', borderRadius: 6,
                  }}>推荐</span>}
                  <span style={{ fontSize: 14, fontWeight: 700 }}>#{p.id} {p.title}</span>
                  <span style={{
                    fontSize: 9, fontWeight: 700,
                    color: STRATEGY_COLORS[p.strategy] || 'var(--text-m)',
                    background: `${STRATEGY_COLORS[p.strategy] || 'var(--text-l)'}18`,
              ...
              {isExpanded && (
                <div style={{
                  ...
                  <p style={{ fontSize: 12, color: 'var(--text-m)', lineHeight: 1.7, margin: '12px 0' }}>
                    {p.description}
                  </p>
                  <div style={{ fontSize: 11, color: 'var(--dust)', marginBottom: 12 }}>
                    🎨 {p.keywords}
                  </div>
                  {/* Score bars */}
 ...
 *  Step 4: 预览与下载
 * ══════════════════════════════════════════════════ */
function PosterPreview({ proposal, journal, onBackToProposals, onFinish }) {
  ...
  // Demo poster URLs — use existing demo assets
  const posterUrls = {
  ...
  useEffect(() => {
    const steps = [
      { time: 1500, text: '正在绘制封面...' },
      { time: 3500, text: '正在精修画面细节...' },
    ];
    const t = setTimeout(() => setGenerating(false), 4000);
    return () => clearTimeout(t);
  }, []);
  ...
  if (generating) {
    return (
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '60px 24px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--text-l)', marginBottom: 24 }}>
          方案 #{proposal.id} · {proposal.title} · {journalLabel}
        </p>
        <div style={{ height: 6, background: 'var(--bg2)', borderRadius: 3, maxWidth: 320, margin: '0 auto' }}>
          <div style={{
            height: '100%', borderRadius: 3, background: 'var(--sage)',
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '12px 24px' }}>
          ...
          {POSTER_LAYOUTS.map((layout) => (
            <button
              ...
              style={{
                ...
                color: activeLayout === layout.key ? '#fff' : 'var(--text-m)',
                background: activeLayout === layout.key ? 'var(--sage)' : 'transparent',
              }}
            >{layout.label}</button>
          ))}
        </div>
        <div style={{ fontSize: 10, color: 'var(--text-l)' }}>
          {currentLayout?.meta} · {currentLayout?.size}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 320px', gap: 16 }}>
        ...
        <div style={{
          borderRadius: 14, overflow: 'hidden',
          border: '1px solid var(--border)', background: '#F0EEEB',
          ...
          <div style={{
            ...
            {regenerating && (
              <div style={{
                ...
                background: 'rgba(240,238,235,0.85)', backdropFilter: 'blur(6px)',
            ...
            <img
              src={posterUrls[activeLayout]}
              alt={`海报 ${currentLayout?.label}`}
              onClick={() => setPreviewUrl(posterUrls[activeLayout])}
              style={{
                maxWidth: '100%', maxHeight: '56vh',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              <span style={{
                fontSize: 9, fontWeight: 700,
                color: STRATEGY_COLORS[proposal.strategy],
                background: `${STRATEGY_COLORS[proposal.strategy]}18`,
                padding: '2px 8px', borderRadius: 6,
              }}>{proposal.strategy}</span>
            </div>
            <div style={{ fontSize: 11, color: 'var(--text-m)' }}>
              {journalLabel} · {proposal.score} 分
            </div>
          </div>
          ...
          {qualityWarning && (
            <div style={{
              ...
              background: 'rgba(172,128,74,0.08)', border: '1px solid rgba(172,128,74,0.2)',
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {POSTER_LAYOUTS.map((layout) => (
                <a
                  key={layout.key}
                  href={posterUrls[layout.key]}
                  download={`poster_${layout.key}.png`}
                  style={{
                    flex: 1, padding: '8px 4px', borderRadius: 8,
                    border: '1px solid var(--border)', background: 'var(--bg)',
                    textAlign: 'center', textDecoration: 'none', color: 'var(--text-d)',
          ...
          <div style={{
            ...
            <div style={{ fontSize: 10, color: 'var(--text-l)', lineHeight: 1.6, marginBottom: 10 }}>
              文字和图片分层，可自由修改标题、作者、字体
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button
          ...
          {showModifyPanel && (
            <div style={{
              borderRadius: 12, padding: '14px',
              border: '1px solid var(--lav)', background: 'var(--card)',
              boxShadow: 'var(--shadow)',
              animation: 'slideDown 0.25s ease',
            }}>
                ...
                {MOCK_SUGGESTIONS.map((s, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      border: adopted ? '1px solid var(--sage)' : '1px solid var(--border)',
                      background: adopted ? 'var(--sage-bg)' : 'var(--bg)',
                      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    }}>
                      <div style={{ fontSize: 11, color: 'var(--text-m)' }}>
                        {s.icon} {s.text}
                      ...
                      <button
                        onClick={() => setAdoptedSuggestions(prev => {
                        ...
                        style={{
                          ...
                          fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
                          background: adopted ? 'var(--sage)' : 'transparent',
                          color: adopted ? '#fff' : 'var(--sage)',
                        }}
                      >{adopted ? '已采纳 ✓' : '采纳'}</button>
                    </div>
                  );
                })}
              </div>
              ...
              <textarea
                ...
                placeholder="我希望背景换成深蓝色，主体的结构再大一点..."
                style={{
                  width: '100%', minHeight: 56, padding: '8px 10px', borderRadius: 8,
                  border: '1px solid var(--border)', background: 'var(--bg)',
                  fontSize: 11, fontFamily: 'inherit', resize: 'vertical', lineHeight: 1.5,
                }}
              />
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '10px 0 8px' }}>
                <button
                  ...
                  style={{
                    border: '1px solid var(--border)', background: 'var(--bg)',
                    borderRadius: 8, padding: '6px 14px', fontSize: 10, fontWeight: 600,
                    color: 'var(--text-m)', cursor: 'pointer', fontFamily: 'inherit',
                  }}
                >🔄 换一个创意方案</button>
                <span style={{ fontSize: 10, color: 'var(--text-l)' }}>
                  剩余免费修改：{freeRetries}/2
                </span>
              </div>
              ...
              <button
                ...
                style={{
                  width: '100%', padding: '11px', borderRadius: 10,
                  background: 'var(--lav)', border: 'none',
      ...
      {showPaywall && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 120, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div onClick={() => setShowPaywall(false)}
            style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} />
          <div style={{
            position: 'relative', zIndex: 1, width: '92%', maxWidth: 380,
            borderRadius: 18, border: '1px solid var(--border)',
            background: 'var(--card)', boxShadow: '0 16px 48px rgba(0,0,0,0.2)',
            padding: '24px 26px', textAlign: 'center',
          }}>
            <div style={{ fontSize: 36, marginBottom: 12 }}>🔒</div>
            <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 8 }}>解锁可编辑版</div>
            <div style={{ fontSize: 12, color: 'var(--text-m)', lineHeight: 1.6, marginBottom: 16 }}>
              可编辑版提供 5 层分离的 PPTX 和矢量 PDF，<br/>
              可自由修改文字、字体和布局。
            </div>
            <div style={{
              padding: '12px', borderRadius: 10, background: 'var(--bg)',
              border: '1px solid var(--border)', marginBottom: 16,
              fontSize: 22, fontWeight: 700, color: 'var(--sage)',
            }}>¥3 <span style={{ fontSize: 12, fontWeight: 400, color: 'var(--text-l)' }}>/ 份</span></div>
            <div style={{ fontSize: 10, color: 'var(--text-l)', marginBottom: 16 }}>
              会员用户可直接下载，无需额外付费
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowPaywall(false)} style={{
                ...
                border: '1px solid var(--border)', background: 'var(--bg)',
                color: 'var(--text-m)', fontSize: 13, fontWeight: 600,
                cursor: 'pointer', fontFamily: 'inherit',
              }}>取消</button>
              <button onClick={() => setShowPaywall(false)} style={{
                flex: 1, padding: '11px', borderRadius: 10,
      ...
      {/* Full-screen poster preview modal */}
      {previewUrl && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 130, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{
            position: 'relative', zIndex: 1, width: 'min(960px, 92vw)',
            background: '#12100F', border: '1px solid #2E2A27',
            borderRadius: 16, overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.35)',
          }}>
            <div style={{
              padding: '10px 14px', borderBottom: '1px solid #2E2A27',
              color: '#D4CEC4', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12, fontWeight: 700,
            }}>
              海报预览 · {currentLayout?.label}
              <button onClick={() => setPreviewUrl(null)} style={{
                border: '1px solid #3F3A36', background: 'transparent',
                color: '#BDB6AB', borderRadius: 8, fontSize: 11, padding: '3px 9px',
                cursor: 'pointer', fontFamily: 'inherit',
              }}>关闭</button>
            </div>
 ...
 *  主容器：PosterFlow
 * ══════════════════════════════════════════════════ */
export default function PosterFlow({
  onNext,
  ...
  const handleStyleSelect = (styleId) => {
    ...
    if (styleId === 'journal') {
      setSubStep(1); // go to journal select
    }
  };

  const handleJournalSelect = (journalId) => {
    setSelectedJournal(journalId);
    setSubStep(2); // go to proposals
  };

  const handleProposalSelect = (proposal) => {
    setSelectedProposal(proposal);
    setSubStep(3); // go to preview
  };
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      {subStep === 3 && (
        <PosterPreview
          ...
          journal={selectedJournal}
          onBackToProposals={() => setSubStep(2)}
          onFinish={onNext}
        />
      )}
